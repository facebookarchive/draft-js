...
var ContentBlocksBuilder = /*#__PURE__*/function () {
  ...
  */
  ...
  _proto._addTableNode = function _addTableNode(tableRoot, style) {
    ...
    var trList = [].concat(tableRoot.querySelectorAll('tr')).map(function (trRoot) {
      return [].concat(trRoot.querySelectorAll('.brick-table-td'));
    });
        ...
        if (tdRoot && tdRoot.rowSpan && tdRoot.colSpan && (tdRoot.rowSpan > 1 || tdRoot.colSpan > 1)) {
          for (var i = 0; i < tdRoot.rowSpan; i++) {
            if (i === 0) {
              ...
              (_trList = trList[index + i]).splice.apply(_trList, [indexCol, 1, trList[index + i][indexCol]].concat(Array(tdRoot.colSpan - 1).fill(null)));
            } else {
              var _trList2;

              (_trList2 = trList[index + i]).splice.apply(_trList2, [indexCol, 1].concat(Array(tdRoot.colSpan).fill(null), [trList[index + i][indexCol]]));
            }
          }
        }
