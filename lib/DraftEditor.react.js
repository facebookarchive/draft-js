...
var DraftEditor = /*#__PURE__*/function (_React$Component2) {
  ...
  function DraftEditor(props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "restoreFiber", function (key) {
      console.log(_this.props.editorRef);
    });

    _defineProperty(_assertThisInitialized(_this), "restoreBlockDOM", function (offsetKeyList, deleteKey, emptyKey, scrollPosition) {
      var blockKeyMap = _this.state.blockKeyMap;
      var newBlockKeyMap = blockKeyMap;
      offsetKeyList.forEach(function (key) {
        newBlockKeyMap = newBlockKeyMap.set(key, blockKeyMap.has(key) ? blockKeyMap.get(key) + 1 : 1);
      });

      var deleteFiberByKey = function deleteFiberByKey(root, deleteKey, isDeleteEmpty) {
        var res = false;
          ...
          if (root.sibling) {
            if (deleteFiberByKey(root.sibling, deleteKey, isDeleteEmpty)) {
              if (isDeleteEmpty) {
                ...
                var emptyFiber = root.sibling.child;
          ...
          if (root.child) {
            if (deleteFiberByKey(root.child, deleteKey, isDeleteEmpty)) {
              if (isDeleteEmpty) {
                ...
                var _emptyFiber = root.child.child;

                while (_emptyFiber.child) {
                  _emptyFiber = _emptyFiber.child;
                }

                _emptyFiber["return"].child = null;
              } else {
                root.child = null;
              }
            }
      ...
      var fiberRoot = document.getElementById('root')._reactRootContainer._internalRoot.current;
